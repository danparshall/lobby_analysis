...
- **Newmark 2017 `TimeThreshold` structured-value support in v2 compendium loader.** The current `load_v2_compendium()` returns `list[dict[str, str]]` — raw strings. Newmark 2017's `lobbyist_registration_threshold_time_percent` cell carries a structured `{magnitude, unit}` value that won't survive a string round-trip. Either the projection module parses the string at read-time (likely YAGNI option) OR `extraction-harness-brainstorm` (Track B) ships v2 Pydantic models with typed cell support before Newmark 2017 implements (currently no schedule). Flag for Stream 2 plan-drafting: plan should explicitly state how `TimeThreshold` is handled (parser in projection module, or wait for typed models).
