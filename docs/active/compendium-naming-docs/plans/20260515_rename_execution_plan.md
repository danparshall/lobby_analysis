...
**E2. The `_dollars` / `_percent` unit-suffix convention in Candidate 3 is novel for the `lobbyist_registration_*` family.** It mirrors `lobbyist_filing_de_minimis_threshold_dollars` (precedent) but introduces drift inside `lobbyist_registration_*`. Acceptable per the walkthrough; flag at execution if the drift turns out to be uglier in practice than expected.
