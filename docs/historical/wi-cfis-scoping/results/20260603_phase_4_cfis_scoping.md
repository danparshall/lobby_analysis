...
The catch: **basic-tier API quota is much tighter than the TOS's "1,000 records/year" reads.** Dan's account exhausted basic-tier quota after ~15 queries in this scoping session and is now flagged for Institute review with a 2-business-day Institute-initiated SLA per the response message. **Phase 0 of the implementation branch is a calendar wait, not a proactive application** — the Institute contacts the account holder; the user replies with affiliation context. The project's qualifying argument is the **Corda Democracy Fellowship at Analogy Group** (Dan is a Fellow there) + non-commercial open-source research + a concrete published deliverable already on main (the 115K-row chain TSV), so this is gating rather than a real risk.
...
3. **When the Institute's review email arrives**, reply with affiliation context: Fellow with the Corda Democracy Fellowship at Analogy Group; project = `lobby_analysis` ("LobbyView for 50 states", open-source non-commercial research filling the state-side gap MIT's LobbyView and OpenSecrets's own 31-state summary leave open); planned use ≈ a few weeks of cycle-contribution pulls for priority states (5-8 states, sitting legislators, ~165 entities per state order-of-magnitude); attribution per CC BY-NC-SA 3.0 US.
...
2. **Project-eligibility self-assessment:** The qualifying argument is the Corda Democracy Fellowship at Analogy Group (a public-research fellowship Dan is a Fellow of) + open-source non-electoral research framing + a concrete published deliverable already on main (`releases/wi/chain/`). FTM grants expanded access generously to research projects of this shape; an exact "academic institution / journalism / 501(c)" status hit is not strictly required when the project framing is clean.
