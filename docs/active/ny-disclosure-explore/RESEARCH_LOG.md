...
> **HANDOFF (next session) — v1.1 `&amp;`-decode bug fixed + chain regenerated; `parties_lobbied` reconnaissance done. Next = `parties_lobbied` resolution (re-pull + name resolution, with Dan), then cosponsors, then multi-year backfill.**
>
...
- **Next:** `parties_lobbied` resolution (with Dan), then cosponsors, then multi-year backfill.
