...
2. **0 gift-event rows.** OH AER Sections II.A (gifts) and II.B (meals) returned zero events across all 316 cached filings. Likely cause is sampling-window / nil-rate artifact (53% of filings have no activity at all); **secondary hypothesis** is that the extraction prompt at `src/lobby_analysis/oh_portal/extraction_brief.py` may not be reading II.A/B fully — investigation tracked separately, not blocking this release. The `gifts/` README documents the empty state honestly.

3. **18 chain rows are extraction defects** (`bill_class == "unmatched"`). Subject text in `bill_reference` field, OAC variants the regex doesn't cover (colons, "Chapter" prefix), or malformed identifiers. **Surfaced, not hidden.** Filter `WHERE confidence = 'unmatched'` to inspect them as a quality canary.
