...
> **HANDOFF (next session):** Phase 2 is underway — the load-bearing **grain-collapse** guard (`io/ny/grain.py`, `collapse_to_filing_grain`) and the per-dataset **column map** (`io/ny/columns.py`, `normalize_columns`, two core datasets) are done and green (13 tests; commit pending). Pick up at the **parser step** (Phase 2 remainder) in [`plans/ny_disclosure_pipeline.md`](plans/ny_disclosure_pipeline.md): derive the canonical `bill_id` — **resolve the State-Bill/level scoping question first** (Phase-0's `starts_with(level,'State')` filter wrongly drops `State Bill` rows filed at `Both` level, e.g. `S550-A`; likely use `focus_type=='State Bill'` alone) — then parse entity + filing/linkage datasets to Pydantic models against the Phase-0 fixtures, then `materialize_ny` + CLI. Build 2025 first via `client_semiannual` (qym9-xzj6). **Plan correction landed this session:** the plan's "dedup per `form_submission_id`" was wrong (no-op + double-counts superseded amendments, 4.1× on the worst real key); verified fix = keep `max(form_submission_id)` per business key `(year, period, principal_lobbyist, beneficial_client, contractual_client_name)`. Documented on GH [#37](https://github.com/danparshall/lobby_analysis/issues/37) (now covers both the cross-dataset *and* within-dataset double-count axes). Baseline: clean green on this machine is **1659** post-Phase-2; 3 pre-existing `scoring` reds tracked in GH [#38](https://github.com/danparshall/lobby_analysis/issues/38) (not NY-scoped).
