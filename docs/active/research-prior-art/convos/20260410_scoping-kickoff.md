...
| Ornstein 2025 — fuzzylink (Probabilistic Record Linkage Using Pretrained Text Embeddings), *Political Analysis* | https://joeornstein.github.io/publications/fuzzylink.pdf | LLM-embedding record linkage; directly applicable to our entity resolution layer. Outperforms fastLink on nickname/abbreviation cases |
| Enamorado, Fifield & Imai 2019 — fastLink, *APSR* 113(2):353–371 | https://imai.fas.harvard.edu/research/files/linkage.pdf | Classical Fellegi-Sunter probabilistic record linkage. Baseline to compare fuzzylink against for corporate entity matching |
...
### Bonus candidate (newly surfaced this session)

- **Libgober & Jerzak 2024 — "Linking Datasets on Organizations Using Half A Billion Open Collaborated Records"**, *Political Science Research & Methods*. This is specifically about organization linkage (not person linkage) and may be even more directly applicable to corporate entity matching in lobbying data than Ornstein's fuzzylink, which focuses on person-name cases. Worth prioritizing.
...
- **Entity resolution across states.** fuzzylink (LLM embeddings) vs. fastLink (classical Fellegi-Sunter) vs. Libgober-Jerzak (OpenCorporates-backed) is an unsettled methodological choice. Depends on how much lexical vs. semantic variation exists in corporate names across state filings — another empirical question.
