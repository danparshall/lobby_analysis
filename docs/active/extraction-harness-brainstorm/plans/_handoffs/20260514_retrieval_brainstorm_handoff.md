...
3. **Re-engage the user on Q1/Q3/Q6.** They wanted to clarify. The four hunches I floated in the prior session's last message (paraphrased below) are starting material:
   - **(i) Per-chunk retrieval may be cheap under prompt caching.** If the statute lives in the cached `system` block, each chunk's retrieval call re-reads the statute at cache-hit rate, not full-token rate. This makes per-chunk dispatch (option (a) of Q1) more attractive than I initially weighted.
   ...
   - **(iv) Output schema may not need cell-level affected list.** If retrieval is per-chunk, the chunk_id is known from the dispatch context. The output's `cell_ids_affected` field becomes redundant; `relevance` (the existing free-text field) is enough. This collapses Q2 too.
