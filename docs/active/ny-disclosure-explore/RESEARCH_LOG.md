...
> **HANDOFF (next session) — Phase 4 DONE + validated; `releases/ny/chain/NY_chain_2025.tsv` shipped. Next = v1.1 enhancements (top: `parties_lobbied` edge) and/or multi-year backfill.**
>
> Phase 4 built the end-to-end **company → lobbyist → bill → lawmaker → $** chain (`allocation/ny/chain.py` + `cli.py`, no IPF) joining `releases/ny/` to the Open States NY 2025-2026 bundle. Shipped: `releases/ny/chain/NY_chain_2025.tsv` (87,534 rows, **committed** — 53 MB, trips GitHub's >50 MB warning, Dan's explicit call) + `chain/README.md`. NY suite **89 green**, ruff clean.
>
> **Validated:** conservation **$0 delta** — `comp_per_cell` over distinct `(filing_id, lobbyist_id, beneficiary_id, bill_id)` cells = **$153,064,191.00** = the Phase-3 bill-linked total exactly. Bill-id match **99.5% distinct / 99.8% link rows** (vs 81.3% without suffix-stripping). 99.9% of rows resolve to an OS bill+sponsor; 213 distinct sponsors = full NY legislature.
>
...
>
> **Top v1.1 follow-up — `parties_lobbied` as a real disclosed lawmaker edge.** Dan asked whether WI imputes "parties lobbied": **it doesn't** — WI's lawmaker edge is also the bill sponsor; its IPF imputes *hours*, not lawmakers. **NY genuinely discloses `parties_lobbied`** (free-text names/titles), but the 2025 pull was a filtered `$select` of **9 fields only** and did **not** fetch it. So it's a real edge needing a re-pull (extend the `$select` in `scripts/ny_pull_2025.py`) + free-text → `ocd-person` resolution. Not an imputation; a v1.1 ingest. **The current chain's lawmaker = bill primary sponsor (inferred via OS), NOT a disclosed lobbying contact — don't overclaim to policymakers** (stated plainly in `chain/README.md`).
>
> **Other open follow-ups:** decode `&amp;` HTML entities in names; cosponsors as a secondary edge; multi-year backfill (2019→); `LobbyingFiling.total_compensation` `Decimal`-typing pass; `lobbyist_bimonthly` fold-in. GH [#37](https://github.com/danparshall/lobby_analysis/issues/37) (amendment dedup), [#38](https://github.com/danparshall/lobby_analysis/issues/38) (3 non-NY scoring reds — leave), [#39](https://github.com/danparshall/lobby_analysis/issues/39) (download_bulk_csv gap).
