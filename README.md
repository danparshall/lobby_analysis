...
Group project of the **Corda Democracy Fellowship** ([project page](https://cordademocracy.org/project-p7.html)). Project lead: Suhan Kacholia (Co-Founder, Analogy Group). Multiple fellows contribute to this repo.
...
- **Pull pipelines.** LLM-driven extraction of structured filings (lobbyist registrations, expenditure reports, contact logs) from state portals into a uniform schema. Knowing what *should* exist per the compendium makes downstream extraction substantially more tractable. The Justia statute-text retrieval pipeline (multi-vintage × 50 states, the substrate for multi-rubric calibration) is documented at [`docs/active/api-multi-vintage-retrieval/README.md`](docs/active/api-multi-vintage-retrieval/README.md).
