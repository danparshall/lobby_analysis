...
| Dollar conservation | Compensation attaches at **filing grain**, replicated across rows. Conservation invariant: sum of distinct-filing comp == filing total; **never** sum raw linkage rows. Per-bill dollar split (a filing covers multiple bills) is a **modeling choice** — recommend recording filing-level comp on each chain row + a `n_bills_in_filing` column so consumers can divide if they want (the NY analog of WI's `modeled_hours_per_sponsor`), rather than fabricating a per-bill dollar. |
