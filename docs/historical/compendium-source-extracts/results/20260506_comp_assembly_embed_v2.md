# Compendium assembly via embeddings — v2  (20260506)

> **Caveat:** one candidate item set produced by ONE assembly method
> (embedding-based coverage analysis). NOT the comprehensive set / NOT
> THE answer for compendium 2.0. Parallel assemblies
> (`comp_assembly_regex`, possibly others) will produce alternate
> candidate sets; the comprehensive set is whatever falls out of
> comparing/reconciling those, not what any single method produces.

Assembled from per-rubric source extracts shipped 2026-05-03, a
coverage-gap extension from Opheim 1991 (identified via OpenAI
`text-embedding-3-large` similarity analysis), and all PRI 2010 items
(added 2026-05-06 per explicit user clearance to use PRI as a
non-default coverage source).
...
- TSV: `20260506_comp_assembly_embed_v2.tsv`  (209 rows × 13 cols)
