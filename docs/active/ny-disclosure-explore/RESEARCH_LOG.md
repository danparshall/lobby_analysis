...
>
> **Your pickup — Phase 4:** chain composer (`allocation/ny/chain.py`, no IPF) joining `NY_filing_bill_links.tsv` → Open States/Plural sponsors. **Two decisions landed 2026-06-05 (Dan) — now in the plan as Decisions 7 & 8:** (1) **split coalition `beneficial_client` cells** into one client per beneficiary and **allocate credit evenly** — composes multiplicatively with the per-bill split (`comp_per_cell = C / (M_beneficiaries · N_bills)`, conserves `C`); a chain-layer transform, the Phase-3 release entity tables keep the raw string. (2) **bill-number padding → defer to Open States' `identifier` format**, don't invent our own canonicalization — so **acquire the OS NY bulk CSV first**, read its real identifiers, then write the normalizer to match (strip `-A/-B`; measure match both ways). Coverage context: State Bill = 87.7% of rows, 85.4% parse to a `bill_id`; some non-parsers embed bills in prose ("(S8417/A8888)") — an extraction opportunity.
>
> **Open follow-ups (not blockers):** `&amp;` HTML entities undecoded in some names; `LobbyingFiling.total_compensation` `Decimal`-typing pass; whether to fold in `lobbyist_bimonthly` (expenses + individual people) and multi-year. GH [#37](https://github.com/danparshall/lobby_analysis/issues/37) (amendment dedup), [#38](https://github.com/danparshall/lobby_analysis/issues/38) (3 non-NY scoring reds — leave), [#39](https://github.com/danparshall/lobby_analysis/issues/39) (download_bulk_csv gap).
