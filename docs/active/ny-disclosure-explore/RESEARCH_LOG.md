...
- **Origin:** Sketched during the `leave-behind-prep` 2026-06-06 take-stock session, after the nickname matcher landed and the lobbyist↔lawmaker disclosed-contact edge hit 98.61% / 213-of-213 legislator coverage. Dan asked for a sketch plan to "complete the chain"; landed on this branch (not leave-behind) per the doc-system-is-persistent-memory norm.
- **Shape:** integrate `parties_lobbied` into the chain as **additive metadata columns** (do NOT replace the inferred-via-sponsor lawmaker leg). Four phases: Phase 0 grain-join sanity gate (~30 min); Phase 1 TDD `disclosed_lawmakers` column (~2-3 hr); Phase 2 TDD reconciliation columns (`sponsor_in_disclosed_set` Boolean + `disclosed_only_lawmaker_count` Int capturing the "leadership contacted but didn't sponsor" signal — Heastie/Stewart-Cousins/Krueger/Gianaris); Phase 3 regen + README + STATE_COVERAGE update. Cost $0 (deterministic joins only).
- **Explicit guardrails:** add-don't-replace; no per-(lawmaker, bill) tuple recovery (Phase-0 NY gating settled this is cartesian-not-mapping); no NULL-bill rows; no cross-state imports (the 2026-06-06 audit confirmed zero today, preserve that). Phase 4 deferrals listed for visibility (target_kind taxonomy, cosponsors, lobbyist_bimonthly, multi-year backfill).
- **4 open questions for Dan** in the plan: column naming, deterministic sort order, parallel disclosed-only summary y/n, STATE_COVERAGE corroboration-metric backfill y/n.
- **Next:** future agent (Dan-in-the-loop or not) executes the plan TDD. No code changes this entry — just planning.
