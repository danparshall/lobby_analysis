...
- [ ] Acquire the NY Open States/Plural bulk CSV (https://open.pluralpolicy.com/data/session-csv/) for the matching session(s) **first** — inspect OS's actual NY bill `identifier` format before writing the normalizer, since the normalizer must match OS's convention (see Decision 8). Stage under `data/bills/NY/<year>/` like WI. Record `ocd-person` sponsor-id coverage.
- [ ] Write the failing bill-id normalizer test (real NY `bill_id`s → the Open States key). **Normalize to whatever OS does** — strip the `-A/-B` print suffix, and match OS's zero-padding/spacing convention exactly (DO NOT invent a separate padding scheme: the source has inconsistent padding — `A00804-C` vs `A804` vs `A1001` — so canonicalize *to OS's form*, measured against the real OS identifiers). Measure match rate with vs. without the suffix.
- [ ] Implement the normalizer. Green.
- [ ] **Split coalition `beneficial_client` cells** (semicolon-delimited beneficiary lists → one client per beneficiary) in the chain composer, and **allocate credit evenly across the split beneficiaries** (Decision 7). The split composes multiplicatively with the per-bill even-split: a filing with comp `C`, `M` beneficiaries, `N` bills emits `C/(M·N)` per `(beneficiary, bill)` cell, so the no-loss invariant still holds (`M·N` cells sum to `C`). Carry `n_beneficiaries_in_filing` alongside `n_bills_in_filing` so a consumer can re-aggregate. (The Phase-3 `releases/ny/` entity tables keep the raw disclosed `beneficial_client` string for source fidelity; the split is a chain/allocation-layer transform, not a re-cut of the release — revisit if we want split clients in `NY_clients.tsv` too.)
- [ ] Write the failing chain-composer + no-loss-invariant tests (fixtures), including a multi-beneficiary × multi-bill filing whose `C/(M·N)` cells sum exactly to `C`.
- [ ] Implement `materialize_chain` (no IPF): split beneficiaries → join `releases/ny/` linkage rows → Open States sponsors on the **OS-normalized** bill key; one row per `(period, principal, lobbyist, beneficiary, bill, sponsor)`; emit `comp_per_cell = filing_compensation / (n_beneficiaries · n_bills)` + `filing_compensation` + `n_beneficiaries_in_filing` + `n_bills_in_filing` + `bill_print_version` + source `unique_id` for traceability; deterministic sort; unmatched-bill rows flagged not dropped. Green.
...
7. **Coalition `beneficial_client` splitting — DECIDED 2026-06-05 (Dan).** Some filings pack many beneficiaries into one semicolon-delimited `beneficial_client` cell. **Split them into separate client entities, and allocate credit evenly across the split beneficiaries** (uniform, like the per-bill split — NY discloses no per-beneficiary weight). Composes multiplicatively with the per-bill even-split: `comp_per_cell = C / (M_beneficiaries · N_bills)`, conserving `C`. This is a Phase-4 chain/allocation-layer transform; the Phase-3 release entity tables keep the raw disclosed string for source fidelity (open: whether to also split `NY_clients.tsv`).
8. **Bill-number zero-padding — DECIDED 2026-06-05 (Dan).** The source is inconsistently padded (`A00804` vs `A804` vs `A1001`), which forks the same bill into distinct `bill_id`s. **Defer to whatever Open States does** — canonicalize NY identifiers to OS's `identifier` format rather than inventing our own padding scheme. Acquire the OS NY bulk CSV first and read its real bill identifiers before writing the normalizer.
