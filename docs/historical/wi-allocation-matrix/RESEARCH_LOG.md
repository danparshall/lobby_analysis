...
- [`results/20260602_unknown_chamber_audit.md`](results/20260602_unknown_chamber_audit.md) — **Unknown-chamber audit.** Phase 3.1's 1,288 hr "unknown" bucket was inflated by a fragile surname-based join; the correct (ID-based) figure is 590 hr / 1.2%, all Joint Legislative Council (one collective entity, 22 bills). Law Revision Committee — hypothesized to contribute to the bucket — has zero chain rows. The missing 698 hr were 3 disambiguated-prefix legislators (B. Jacobson, L. Johnson, J. Jacobson) whose `ocd-person/...` IDs resolve correctly. Synthesis chamber-rollup numbers corrected: Assembly 25,892 → 26,543; Senate 21,610 → 21,657; ratio 1.20× → 1.23×. Substantive finding (chamber-bias reversal after per-sponsor normalization) unchanged. Production code only writes the name column, doesn't join on it — no code change needed.
