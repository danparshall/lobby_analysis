...
### 2026-06-12 — Merge `origin/main` + finish-branch flow (executed by Dan's agent, not Amina's)

- **Convo:** [`convos/20260612_merge_main_and_finish_branch.md`](convos/20260612_merge_main_and_finish_branch.md)
- **What happened.** PR #56 was DIRTY/CONFLICTING after main absorbed Dan's NY-disclosure-explore merge (`79cc7a2`) on top of prior wi-ralph + oh-portal-aprime-batch + leave-behind-prep activity. Only `STATUS.md` conflicted (Last updated header + Active Research Lines table + Recent Sessions, 3 regions); resolved by preserving both sets of notes per Dan's standing directive. Dan invoked `finishing-a-research-branch` to close out the branch.
- **No code changed in this session.** The frontend explorer + `/stats` endpoint substance landed in Amina's 2026-06-11 session; this session just resolved STATUS.md conflicts and ran the archive flow.
- **Test caveat.** The full repo suite fails when local Postgres isn't running because `tests/conftest.py`'s autouse `_truncate_filings` fixture requires a DB connection for *every* test in the suite, not just backend tests. Not a regression introduced by this session; flagged as a follow-up cleanup (scope the fixture to a `pytest.mark.backend` marker or move it to a `tests/backend/conftest.py`).
- **Outcome.** Merge commit + archive commit landed; branch archived to `docs/historical/backend-prototype/`; PR #56 ready for merge.
