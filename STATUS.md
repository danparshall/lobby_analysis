...
|--------|---------|-------|--------|
| pri-2026-rescore | 2026-04-12 | Re-score all 50 state lobbying portals against PRI's 22 accessibility + 37 disclosure-law criteria (2026 vintage) using a Sonnet-subagent scoring pipeline | Phases 1–4 data-collection complete: rubrics locked; 50/50 states have seed URL JSONs and raw portal snapshots (981 artifacts, ~350 MB, ~10 states need playwright follow-up for SPA/WAF captures). Next: Phase 5 unified PRI+FOCAL scoring (see `docs/active/pri-2026-rescore/plans/20260414_pri_focal_unified_scoring_handoff.md`). |
| focal-extraction | 2026-04-13 | Extract FOCAL 2024's 50 indicators into machine-readable form, then score all 50 states against FOCAL in parallel with PRI | Extraction complete (CSV + methodology note shipped). **Scoring pipeline unified with PRI (2026-04-14 decision)** — both rubrics will be scored against the same snapshot corpus in one chained pass per state. Next: branch strategy + Phase 5 execution per the handoff doc. |
